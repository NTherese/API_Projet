/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package Pharmacie.graph;

import Pharmacie.DAO.MedecinDAO;
import Pharmacie.DAO.MedicamentDAO;
import Pharmacie.DAO.PatientDAO;
import Pharmacie.DAO.PrescriptionDAO;
import connections.DBConnection;
import java.awt.CardLayout;
import java.sql.Connection;
import javax.swing.JOptionPane;

/**
 *
 * @author meril
 */
public class Gestion_Pharmacie extends javax.swing.JFrame {

    CardLayout cardl;
    /**
     * Creates new form Gestion_Pharmacie
     */
    public Gestion_Pharmacie() {
        initComponents();
        cardl=(CardLayout)this.getContentPane().getLayout();
        Connection dbConnect = DBConnection.getConnection();
        if (dbConnect == null) {
            System.out.println("connection invalide");
            JOptionPane.showMessageDialog(this,"connexion invalide","ERREUR",JOptionPane.ERROR_MESSAGE);
        }
        PatientDAO patDAO=new PatientDAO();
       patDAO.setDbConnect(dbConnect);
       
       PrescriptionDAO presDAO= new PrescriptionDAO();
      presDAO.setDbConnect(dbConnect);
      
      MedecinDAO medDAO=new MedecinDAO();
      medDAO.setDbConnect(dbConnect);
      
      MedicamentDAO medicDAO=new MedicamentDAO();
      medicDAO.setDbConnect(dbConnect);
      
      creationPres1.setPrescriptionDAO(presDAO);
      rechPres1.setPrescriptionDAO(presDAO);
      creationPres1.setMedecinDAO(medDAO);
      creationPres1.inject_medecin();
      creationPres1.setPatientDAO(patDAO);
      creationPres1.inject_patient();
      
      creation1.setPatientDAO(patDAO);
      rechExact1.setPatientDAO(patDAO);
      rechExact1.setPrescriptionDAO(presDAO);
      rechPart1.setPatientDAO(patDAO);
      
      creationMedecin1.setMedecinDAO(medDAO);
      rechPartMedecin1.setMedecinDAO(medDAO);
      rechExactMedecin1.setMedecinDAO(medDAO);
      rechExactMedecin1.setPrescriptionDAO(presDAO);
      
      
      creationMedicaments1.setMedicamentDAO(medicDAO);
      rechPartMedic1.setMedicamentDAO(medicDAO);
      rechExacteMedic1.setMedicamentDAO(medicDAO);
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        font1 = new Pharmacie.graph.font();
        creation1 = new Pharmacie.graph.Creation();
        creationPres1 = new Pharmacie.graph.CreationPres();
        rechExact1 = new Pharmacie.graph.RechExact();
        rechPart1 = new Pharmacie.graph.RechPart();
        rechPres1 = new Pharmacie.graph.RechPres();
        creationMedecin1 = new Pharmacie.graph.CreationMedecin();
        rechExactMedecin1 = new Pharmacie.graph.RechExactMedecin();
        rechPartMedecin1 = new Pharmacie.graph.RechPartMedecin();
        creationMedicaments1 = new Pharmacie.graph.CreationMedicaments();
        rechPartMedic1 = new Pharmacie.graph.RechPartMedic();
        rechExacteMedic1 = new Pharmacie.graph.RechExacteMedic();
        jMenuBar1 = new javax.swing.JMenuBar();
        menuAccueil = new javax.swing.JMenu();
        itemAccueil = new javax.swing.JMenuItem();
        menuPat = new javax.swing.JMenu();
        itemCreaPat = new javax.swing.JMenuItem();
        RechExacte = new javax.swing.JMenuItem();
        RechPartielle = new javax.swing.JMenuItem();
        menuPres = new javax.swing.JMenu();
        itemCreaPres = new javax.swing.JMenuItem();
        itemRechPres = new javax.swing.JMenuItem();
        menuMedecins = new javax.swing.JMenu();
        itemCreaMed = new javax.swing.JMenuItem();
        RechPartMed = new javax.swing.JMenuItem();
        RechExactMed = new javax.swing.JMenuItem();
        menuMedicaments = new javax.swing.JMenu();
        itemCreaMedic = new javax.swing.JMenuItem();
        itemRechPartMedic = new javax.swing.JMenuItem();
        itemRechExacteMedic = new javax.swing.JMenuItem();

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        getContentPane().setLayout(new java.awt.CardLayout());
        getContentPane().add(font1, "cardAccueil");
        getContentPane().add(creation1, "cardCreaPat");
        getContentPane().add(creationPres1, "cardCreationPres");
        getContentPane().add(rechExact1, "cardRechExacte");
        getContentPane().add(rechPart1, "cardRechPart");
        getContentPane().add(rechPres1, "cardRechPres");
        getContentPane().add(creationMedecin1, "CardCreationMedecin");
        getContentPane().add(rechExactMedecin1, "CardRechExactMedecin");
        getContentPane().add(rechPartMedecin1, "CardRechPartMedecin");
        getContentPane().add(creationMedicaments1, "CardCreationMedic");
        getContentPane().add(rechPartMedic1, "cardRecPartMedic");
        getContentPane().add(rechExacteMedic1, "CardRechExacteMedic");

        menuAccueil.setText("Accueil");
        menuAccueil.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                menuAccueilActionPerformed(evt);
            }
        });

        itemAccueil.setText("Accueil Pharmacie");
        itemAccueil.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemAccueilActionPerformed(evt);
            }
        });
        menuAccueil.add(itemAccueil);

        jMenuBar1.add(menuAccueil);

        menuPat.setText("Patients");

        itemCreaPat.setText("Creation Patient");
        itemCreaPat.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemCreaPatActionPerformed(evt);
            }
        });
        menuPat.add(itemCreaPat);

        RechExacte.setText("Recherche Numero Patient");
        RechExacte.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                RechExacteActionPerformed(evt);
            }
        });
        menuPat.add(RechExacte);

        RechPartielle.setText("Recherche partielle patient");
        RechPartielle.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                RechPartielleActionPerformed(evt);
            }
        });
        menuPat.add(RechPartielle);

        jMenuBar1.add(menuPat);

        menuPres.setText("Prescriptions");

        itemCreaPres.setText("Creation  prescription");
        itemCreaPres.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemCreaPresActionPerformed(evt);
            }
        });
        menuPres.add(itemCreaPres);

        itemRechPres.setText("Recherche prescription + vue");
        itemRechPres.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemRechPresActionPerformed(evt);
            }
        });
        menuPres.add(itemRechPres);

        jMenuBar1.add(menuPres);

        menuMedecins.setText("Medecins");

        itemCreaMed.setText("Creation medecin");
        itemCreaMed.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemCreaMedActionPerformed(evt);
            }
        });
        menuMedecins.add(itemCreaMed);

        RechPartMed.setText("Recherche par le matricule");
        RechPartMed.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                RechPartMedActionPerformed(evt);
            }
        });
        menuMedecins.add(RechPartMed);

        RechExactMed.setText("Recherche par l'identifiant");
        RechExactMed.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                RechExactMedActionPerformed(evt);
            }
        });
        menuMedecins.add(RechExactMed);

        jMenuBar1.add(menuMedecins);

        menuMedicaments.setText("Medicaments");

        itemCreaMedic.setText("Creation Medicament");
        itemCreaMedic.setToolTipText("");
        itemCreaMedic.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemCreaMedicActionPerformed(evt);
            }
        });
        menuMedicaments.add(itemCreaMedic);

        itemRechPartMedic.setText("Recherche sur la description");
        itemRechPartMedic.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemRechPartMedicActionPerformed(evt);
            }
        });
        menuMedicaments.add(itemRechPartMedic);

        itemRechExacteMedic.setText("Recherche sur le code + vue quantite ");
        itemRechExacteMedic.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itemRechExacteMedicActionPerformed(evt);
            }
        });
        menuMedicaments.add(itemRechExacteMedic);

        jMenuBar1.add(menuMedicaments);

        setJMenuBar(jMenuBar1);

        pack();
    }// </editor-fold>//GEN-END:initComponents

    private void itemCreaPatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemCreaPatActionPerformed
        cardl.show(this.getContentPane(),"cardCreaPat");
    }//GEN-LAST:event_itemCreaPatActionPerformed

    private void RechExacteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RechExacteActionPerformed
        cardl.show(this.getContentPane(),"cardRechExacte");
    }//GEN-LAST:event_RechExacteActionPerformed

    private void RechPartielleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RechPartielleActionPerformed
        cardl.show(this.getContentPane(),"cardRechPart");
    }//GEN-LAST:event_RechPartielleActionPerformed

    private void itemCreaPresActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemCreaPresActionPerformed
         cardl.show(this.getContentPane(),"cardCreationPres");
    }//GEN-LAST:event_itemCreaPresActionPerformed

    private void itemRechPresActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemRechPresActionPerformed
        cardl.show(this.getContentPane(),"cardRechPres");
    }//GEN-LAST:event_itemRechPresActionPerformed

    private void menuAccueilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuAccueilActionPerformed
        cardl.show(this.getContentPane(),"cardAccueil");
    }//GEN-LAST:event_menuAccueilActionPerformed

    private void itemAccueilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemAccueilActionPerformed
       cardl.show(this.getContentPane(),"cardAccueil");
    }//GEN-LAST:event_itemAccueilActionPerformed

    private void itemCreaMedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemCreaMedActionPerformed
         cardl.show(this.getContentPane(),"CardCreationMedecin");
    }//GEN-LAST:event_itemCreaMedActionPerformed

    private void RechPartMedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RechPartMedActionPerformed
       cardl.show(this.getContentPane(),"CardRechPartMedecin");
    }//GEN-LAST:event_RechPartMedActionPerformed

    private void RechExactMedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RechExactMedActionPerformed
       cardl.show(this.getContentPane(),"CardRechExactMedecin");
    }//GEN-LAST:event_RechExactMedActionPerformed

    private void itemCreaMedicActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemCreaMedicActionPerformed
        cardl.show(this.getContentPane(),"CardCreationMedic");
    }//GEN-LAST:event_itemCreaMedicActionPerformed

    private void itemRechPartMedicActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemRechPartMedicActionPerformed
       cardl.show(this.getContentPane(),"cardRecPartMedic");
    }//GEN-LAST:event_itemRechPartMedicActionPerformed

    private void itemRechExacteMedicActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemRechExacteMedicActionPerformed
       cardl.show(this.getContentPane(),"CardRechExacteMedic");
    }//GEN-LAST:event_itemRechExacteMedicActionPerformed
    
    /**
     * @param args the command line arguments
     */
    public static void main(String args[]) {
        /* Set the Nimbus look and feel */
        //<editor-fold defaultstate="collapsed" desc=" Look and feel setting code (optional) ">
        /* If Nimbus (introduced in Java SE 6) is not available, stay with the default look and feel.
         * For details see http://download.oracle.com/javase/tutorial/uiswing/lookandfeel/plaf.html 
         */
        try {
            for (javax.swing.UIManager.LookAndFeelInfo info : javax.swing.UIManager.getInstalledLookAndFeels()) {
                if ("Nimbus".equals(info.getName())) {
                    javax.swing.UIManager.setLookAndFeel(info.getClassName());
                    break;
                }
            }
        } catch (ClassNotFoundException ex) {
            java.util.logging.Logger.getLogger(Gestion_Pharmacie.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (InstantiationException ex) {
            java.util.logging.Logger.getLogger(Gestion_Pharmacie.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (IllegalAccessException ex) {
            java.util.logging.Logger.getLogger(Gestion_Pharmacie.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (javax.swing.UnsupportedLookAndFeelException ex) {
            java.util.logging.Logger.getLogger(Gestion_Pharmacie.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //</editor-fold>

        /* Create and display the form */
        java.awt.EventQueue.invokeLater(() -> {
            new Gestion_Pharmacie().setVisible(true);
        });
    }

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JMenuItem RechExactMed;
    private javax.swing.JMenuItem RechExacte;
    private javax.swing.JMenuItem RechPartMed;
    private javax.swing.JMenuItem RechPartielle;
    private Pharmacie.graph.Creation creation1;
    private Pharmacie.graph.CreationMedecin creationMedecin1;
    private Pharmacie.graph.CreationMedicaments creationMedicaments1;
    private Pharmacie.graph.CreationPres creationPres1;
    private Pharmacie.graph.font font1;
    private javax.swing.JMenuItem itemAccueil;
    private javax.swing.JMenuItem itemCreaMed;
    private javax.swing.JMenuItem itemCreaMedic;
    private javax.swing.JMenuItem itemCreaPat;
    private javax.swing.JMenuItem itemCreaPres;
    private javax.swing.JMenuItem itemRechExacteMedic;
    private javax.swing.JMenuItem itemRechPartMedic;
    private javax.swing.JMenuItem itemRechPres;
    private javax.swing.JMenuBar jMenuBar1;
    private javax.swing.JMenu menuAccueil;
    private javax.swing.JMenu menuMedecins;
    private javax.swing.JMenu menuMedicaments;
    private javax.swing.JMenu menuPat;
    private javax.swing.JMenu menuPres;
    private Pharmacie.graph.RechExact rechExact1;
    private Pharmacie.graph.RechExactMedecin rechExactMedecin1;
    private Pharmacie.graph.RechExacteMedic rechExacteMedic1;
    private Pharmacie.graph.RechPart rechPart1;
    private Pharmacie.graph.RechPartMedecin rechPartMedecin1;
    private Pharmacie.graph.RechPartMedic rechPartMedic1;
    private Pharmacie.graph.RechPres rechPres1;
    // End of variables declaration//GEN-END:variables
}
